...
- **`vendor/`** holds the third-party libraries the page loads at runtime — `three.min.js` (r128), `SVGLoader`/`GLTFLoader`/`DRACOLoader`, the Draco decoder binaries, and `supabase-js`. Every one is loaded **local-first with a CDN fallback**; nothing depends on a CDN being reachable. Added Aug 19 2026 — before that the local Three.js copy was referenced but never committed, and the loaders plus supabase-js were single-CDN with no fallback and failed *silently*, so a blocked jsdelivr produced a galaxy with every kit box missing its 3D model and no error shown. If you add another third-party script, vendor it and give it the same chain.
